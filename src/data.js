export const PREGUNTAS = [
   {
      texto: "¿Cuáles son los libros más populares de la Biblia?",
      respuestas: [
         { texto: "Génesis", puntos: 40 },
         { texto: "Salmos", puntos: 35 },
         { texto: "Proverbios", puntos: 30 },
         { texto: "Apocalipsis", puntos: 25 },
         { texto: "Juan", puntos: 20 }
      ],
      categoria: "biblia"
   },
   {
      texto: "¿Comida típica mexicana?",
      respuestas: [
         { texto: "Tacos", puntos: 50 },
         { texto: "Tamales", puntos: 30 },
         { texto: "Pozole", puntos: 25 },
         { texto: "Mole", puntos: 20 },
         { texto: "Chiles en nogada", puntos: 15 }
      ],
      categoria: "comida"
   },
   {
      texto: "¿Capital de Francia?",
      respuestas: [
         { texto: "París", puntos: 50 },
         { texto: "Lyon", puntos: 10 },
         { texto: "Marsella", puntos: 5 },
         { texto: "Niza", puntos: 3 },
         { texto: "Toulouse", puntos: 2 }
      ],
      categoria: "geografia"
   },
   {
      texto: "¿Qué animal da leche?",
      respuestas: [
         { texto: "Vaca", puntos: 60 },
         { texto: "Cabra", puntos: 20 },
         { texto: "Oveja", puntos: 10 },
         { texto: "Burro", puntos: 5 },
         { texto: "Ballena", puntos: 0 }
      ],
      categoria: "animales"
   },
   {
      texto: "¿De qué color es el cielo en un día despejado?",
      respuestas: [
         { texto: "Azul", puntos: 80 },
         { texto: "Gris", puntos: 5 },
         { texto: "Naranja", puntos: 5 },
         { texto: "Verde", puntos: 0 },
         { texto: "Negro", puntos: 0 }
      ],
      categoria: "naturaleza"
   },
   {
      texto: "¿Fruta cítrica?",
      respuestas: [
         { texto: "Naranja", puntos: 45 },
         { texto: "Limón", puntos: 40 },
         { texto: "Toronja", puntos: 10 },
         { texto: "Mandarina", puntos: 5 },
         { texto: "Pomelo", puntos: 0 }
      ],
      categoria: "comida"
   },
   {
      texto: "¿Color de la bandera de México?",
      respuestas: [
         { texto: "Verde", puntos: 40 },
         { texto: "Blanco", puntos: 30 },
         { texto: "Rojo", puntos: 30 },
         { texto: "Amarillo", puntos: 0 },
         { texto: "Azul", puntos: 0 }
      ],
      categoria: "geografia"
   },
   {
      texto: "¿Cuál es una serie de streaming popular?",
      respuestas: [
         { texto: "Stranger Things", puntos: 40 },
         { texto: "The Crown", puntos: 20 },
         { texto: "La Casa de Papel", puntos: 30 },
         { texto: "Breaking Bad", puntos: 25 },
         { texto: "Friends", puntos: 35 }
      ],
      categoria: "entretenimiento"
   },
   {
      texto: "¿Deporte con balón?",
      respuestas: [
         { texto: "Fútbol", puntos: 60 },
         { texto: "Baloncesto", puntos: 20 },
         { texto: "Voleibol", puntos: 10 },
         { texto: "Rugby", puntos: 5 },
         { texto: "Golf", puntos: 0 }
      ],
      categoria: "deportes"
   },
   {
      texto: "¿Algo que encuentras en la escuela?",
      respuestas: [
         { texto: "Libros", puntos: 50 },
         { texto: "Pupitres", puntos: 30 },
         { texto: "Maestros", puntos: 40 },
         { texto: "Receso", puntos: 20 },
         { texto: "Exámenes", puntos: 10 }
      ],
      categoria: "educacion"
   }
];
